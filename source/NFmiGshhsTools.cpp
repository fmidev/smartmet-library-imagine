...
namespace Imagine
{
  ...
  namespace NFmiGshhsTools
  {
					  ...
					  double theMinArea)
	{
	  ...
	  if(theMinLongitude >= theMaxLongitude ||
		 theMinLatitude >= theMaxLatitude)
		return ret;
	  ...
	  GSHHS header;
	  POINT point;
	  int n_read = fread(static_cast<void *>(&header),
	  ...
	  while(n_read == 1)
		{
		  ...
		  if(outside || (area<theMinArea && theMinArea>=0))
		  ...
		  else
			{
			  for(int k=0; k<header.n; k++)
				{
				  if(fread(static_cast<void *>(&point),
				  ...
				  const double lon = (header.greenwich && point.x > max ?
									  point.x * 1.0e-6 - 360 :
									  point.x * 1.0e-6);
		  ...
		  n_read = fread(static_cast<void *>(&header),
						 size_t(sizeof(struct GSHHS)),
						 size_t(1),
						 fp);
		}
	  ...
	  ret.Clip(theMinLongitude, theMinLatitude,
			   theMaxLongitude, theMaxLatitude);
	  return ret;
