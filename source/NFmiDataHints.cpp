...
namespace Imagine
{
  ...
  {
  public:
	Pimple(const NFmiDataMatrix<float> & theData,
		   int theMaxSize);
  ...
  NFmiDataHints::Pimple::Pimple(const NFmiDataMatrix<float> & theData,
								int theMaxSize)
									  ...
									  int theMaxSize)
  {
	...
	if( (width<=theMaxSize && height<=theMaxSize) ||
		(width<=1 || height<=1) )
	  {
	...
	else
	  {
		...
		if(width>height)
		  {
			int x = (x1+x2)/2;
			recurse(theNode->itsLeft,
					theData,
					x1,y1,
					x,y2,
					theMaxSize);
			recurse(theNode->itsRight,
					theData,
					x,y1,
					x2,y2,
					theMaxSize);
		...
		theNode->itsRectangle.hasmissing = (theNode->itsLeft->itsRectangle.hasmissing |
											theNode->itsRight->itsRectangle.hasmissing);
  ...
  NFmiDataHints::return_type
  NFmiDataHints::Pimple::rectangles(float theLoLimit,
									float theHiLimit) const
  {
	return_type ret;
								   ...
								   float theHiLimit) const
  {
	...
	if(!haschildren)
	  ...
	  {
		bool leftok = find(theValues,
						   theNode->itsLeft,
						   theLoLimit,
						   theHiLimit);
		bool rightok = find(theValues,
							theNode->itsRight,
							theLoLimit,
							theHiLimit);
		if(leftok && rightok)
  ...
  NFmiDataHints::NFmiDataHints(const NFmiDataMatrix<float> & theData,
							   int theMaxSize)
  ...
  NFmiDataHints::return_type
  NFmiDataHints::rectangles(float theLoLimit,
							float theHiLimit) const
  {
