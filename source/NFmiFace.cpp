...
namespace Imagine
{
  ...
  NFmiFace::NFmiFace(const std::string & theFile,
					 int theWidth,
					 int theHeight)
	: itsFile(theFile)
	, itsWidth(theWidth)
					  ...
					  NFmiColorTools::NFmiBlendRule theRule) const
  {
	NFmiFreeType::Instance().Draw(theImage,
								  itsFile,itsWidth,itsHeight,
								  theX, theY,
								  theText, theAlignment, theColor, theRule,
								  itsBackgroundOn,
								  itsBackgroundWidth, itsBackgroundHeight,
								  itsBackgroundColor, itsBackgroundRule);
  }
