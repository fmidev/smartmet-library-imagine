...
namespace Imagine
{
  ...
  template <typename T>
  inline
  bool bitset(T theByte, T theBit)
  {
  ...
  void read_colormap(FILE * theInput,
					 int theBits,
					 unsigned char theColorMap[3][MaxColors])
  {
	unsigned char rgb[3];
  ...
  int getdatablock(FILE * theInput,
				   unsigned char * theBuffer)
  {
	int count;
  ...
  void read_extension(FILE * theInput,
					  int theLabel,
					  GifScreen & theScreen)
  {
  ...
  int lzwreadbyte(FILE * theInput,
				  bool theFlag,
				  int theInputCodeSize)
  {
	...
	while((v = lzwreadbyte(theInput,false,c)) >= 0)
	  {
		if(v == theTransparent)
		  ...
		  theImage(xpos,ypos) = NFmiColorTools::MakeColor(theColorMap[0][v],
														  theColorMap[1][v],
														  theColorMap[2][v]);
		++xpos;
		if(xpos == theWidth)
