...
ImagineXr::ImagineXr( int w, int h, const string &to_fn, const string &fmt_ ) 
        :fn(to_fn), fmt(fmt_), width(w), height(h)
        ,face(0), font_bg_color(NFmiColorTools::NoColor)
{
USAGE( "ImagineXr(): format=%s, fn=%s width=%d, height=%d", fmt.c_str(), fn.c_str(), width, height );
...
ImagineXr::ImagineXr( const ImagineXr& o )
        :pdf_surf(o.pdf_surf), image_surf(o.image_surf)
        ,fn(o.fn), fmt(o.fmt), width(o.width), height(o.height)
        ,face(0), font_bg_color(NFmiColorTools::NoColor)
{
...
void ImagineXr::SetBlend( enum NFmiColorTools::NFmiBlendRule rule ) {
    ...
    switch( rule ) {
        ...
        */
        case kFmiColorClear: op= OPERATOR_CLEAR; break;     // Fs=0, Fd=0
        case kFmiColorCopy: op= OPERATOR_SOURCE; break;     // Fs=1, Fd=0
        case kFmiColorKeep: op= OPERATOR_DEST; break;       // Fs=0, Fd=1
        case kFmiColorOver: op= OPERATOR_OVER; break;       // Fs=1, Fd=1-As
        case kFmiColorUnder: op= OPERATOR_DEST_OVER; break; // Fs=1-Ad, Fd=1
        case kFmiColorIn: op= OPERATOR_IN; break;           // Fs=Ad, Fd=0
        case kFmiColorKeepIn: op= OPERATOR_DEST_IN; break;  // Fs=0, Fd=As
        case kFmiColorOut: op= OPERATOR_OUT; break;         // Fs=1-Ad, Fd=0
        case kFmiColorKeepOut: op= OPERATOR_DEST_OUT; break;    // Fs=0, Fd=1-As
        case kFmiColorAtop: op= OPERATOR_ATOP; break;       // FS=Ad, Fd=1-As
        case kFmiColorKeepAtop: op= OPERATOR_DEST_ATOP; break;  // FS=1-Ad, Fd=As
        case kFmiColorXor: op= OPERATOR_XOR; break;         // FS=1-Ad, Fd=1-As

        /* Additional rules not by Porter-Duff
        */
        case kFmiColorPlus: op= OPERATOR_ADD; break;        // Fs=1, Fd=1
...
USAGE( "\t\tline_to( %.2f, %.2f )", x, y );
            ...
            //
            default: throw runtime_error( "Unexpected path primitive" );
        }
    }
    return true;
}
...
void ImagineXr::ApplyAlignment( enum NFmiAlignment alignment, int &x, int &y, int w, int h ) {
    switch( alignment ) {
        ...
        case kFmiAlignNorthWest:   break;     // no modifications

        case kFmiAlignCenter:      x -= w/2; y -= h/2; break;
        case kFmiAlignNorth:       x -= w/2; break;
        case kFmiAlignNorthEast:   x -= w; break;
        case kFmiAlignEast:        x -= w; y -= h/2; break;
        case kFmiAlignSouthEast:   x -= w; y -= h; break;
        case kFmiAlignSouth:       x -= w/2; y -= h; break;
        case kFmiAlignSouthWest:   y -= h; break;
        case kFmiAlignWest:        y -= h/2; break;
    }
}
...
static inline NFmiColorTools::Color ARGB32_to_NFmiColor( int32_t v ) {
    ...
    if (a==0xff)
        ...
        if (a==127) a=126;  // we don't come here with full alpha; neither should we leave
        else if (a==0) a=1; // nor opaqueness
...
void ImagineXr::Composite( const ImagineXr &img2,
                ...
                int x, int y, float opaque ) {

USAGE( "Composite(): rule=%s, align=%s, x=%d, y=%d, opaque=%.3f", RULENAME(rule), ALIGNNAME(alignment), x, y, opaque );
...
USAGE( "Stroke(): line_width=%.2f, color=%s, rule=%s", line_width, COLORNAME(color), RULENAME(rule) );
...
USAGE( "Fill(): pattern=yyy, rule=%s, opaque=%.3f", RULENAME(rule), opaque );
    ...
    //
#if 1
    if (opaque==1.0) {  // 308kB without this
        cr->fill();
        return;
    }
#endif
...
* 'fontspec':
*   "<fontname>:<points>"
*   "<fontname>:<width>x<height>"   old format, not supported for Cairo (i.e. "misc/6x13B.pcf.gz:6x13")
*
...
USAGE( "MakeFace(): fontspec=%s, backcolor=%s, xm=%d, ym=%d", fontspec.c_str(), COLORNAME(backcolor), xm, ym );
    ...
    if (strchr( to_vec[1].c_str(), 'x' )) {
        WARNING( "Old style fontspec '%s' replaced with default (use \"<fontname>:<points>\" instead).", fontspec.c_str() );
    } else {
        font_name= to_vec[0];
...
*/
bool ImagineXr::DrawFace( int x_, int y_,
...
USAGE( "DrawFace(): x=%d, y=%d, text=%s, color=%s, align=%s, rule=%s",
    x_, y_, text.c_str(), COLORNAME(color), ALIGNNAME(alignment), RULENAME(rule) );
