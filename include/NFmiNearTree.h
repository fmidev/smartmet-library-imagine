...
#ifndef _NFMINEARTREE_H
#define _NFMINEARTREE_H
...
public:
  ...
  ~NFmiNearTree(void)
  {
	delete itsLeftBranch  ;  itsLeftBranch  =0;
	delete itsRightBranch ;  itsRightBranch =0;
	delete itsLeftObject  ;  itsLeftObject  =0;
	delete itsRightObject ;  itsRightObject =0;
  }
...
private:
  ...
  bool Nearest ( double & theRadius, T & theClosest, const T & thePoint ) const
  {
    ...
    if ( (itsLeftObject!=0) &&
		 ( (tmpradius = Distance(thePoint,*itsLeftObject)) <= theRadius ) )
      {
		theRadius  = tmpradius;
