...
#ifndef _NFMISIMPLIFIER_H
#define _NFMISIMPLIFIER_H
		...
		if(i+1<j)
		  {
			...
			double term3 = theData[i].X()*theData[j].Y() -
			  theData[j].X()*theData[i].Y();
			...
			for(int k=i+1; k<j; k++)
			  {
				// The area
				double tmp = 0.5*(theData[k].Y()*xixj
								  -theData[k].X()*yiyj
								  +term3);
				if(tmp<0) tmp=-tmp;
